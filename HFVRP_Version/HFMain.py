...
functions. OperatorSelection.py contains all weight and score update functions, a
function to select heuristics and vehicle selection mechanism. AcceptanceCriteria.py contains a simple 
...
def increment_teta(teta_dest, teta_rep, teta_noise, teta_explor, chosen_ops):
    #Noise
    ...
    if chosen_ops[1] == 'Random':
        teta_rep[4] += 1
    return teta_dest, teta_rep, teta_noise, teta_explor

aug_scores = dict(enumerate([31, 9, 5])) #Score increasing parameters sigma 1,2 and 3
...
score_explor = [0]*2 #List that keeps track of exploration score
...
w_noise = [1]*2 #First entry is Noise = True, second is Noise = False
w_explor = [1,1] #List that keeps track of exploration score
...
teta_explor = [0]*2 #List that keeps track of number of time exploration is used
...
if choice2 == 0:
    points, data, dist_mat, hub_num, routes, indices, veh_types = HFNewInitialSolutions.import_data(choice3)
    ...
    points, data, dist_mat, hub_num, routes, indices, veh_types = HFNewInitialSolutions.import_data(choice3)
    points, data, indices, inv_points2, dist_mat, hub_num, solution_id_copy, veh_types = HFBenchmarkPreprocess.import_and_process_data(file, file2)
    fleet = veh_types.to_dict()
        ...
        for i in range(1,n_veh+1):
            for j in range(n_points): 
                for k in range(hub_num, n_points):
                    pheromone_mat[i-1][j][k] =  data['weight'][HFRepairOps.find_id(k, points)]/fleet['max_weight'][i] + data['volume'][HFRepairOps.find_id(k, points)]/fleet['capacity'][i]  
...
max_veh_cost = max_cost = max(fleet['variable_cost'][vehicle] for vehicle in list(fleet['variable_cost'].keys())) #Maximum vehicle cost
max_N = n_factor*(max_dist*max_cost) #Noise interval threshold

#Acquire random Initial Solutions
init_sol, init_veh = HFNewInitialSolutions.InitialSolution(points2, data2, indices2, inv_points2, hub_num, dist_mat, choice, fleet, veh_sol, pheromone_mat)
...
best_sol_cost = HFRepairOps.solution_cost(best_sol, dist_mat, points, inv_points2, data, fleet, veh_sol)
...
temperature = HFAcceptanceCriteria.calculate_starting_temperature(best_sol_cost, w)
...
def ALNS_destroy_repair(solution_id_copy, best_sol, points2, inv_points2, data2, dist_mat, hub_num, indices2, chosen_ops, current_iter, iter_threshold, gamma, best_veh_sol, pheromone_mat, max_N, pu_norm, do_norm, max_dist, load_norm):
    #Destroy Solution
    partial_solution, removed_req, partial_veh_solution = HFDestroyOps.DestroyOperator(
    ...
    #Repair Solution
    current_sol, current_veh_sol = HFRepairOps.RepairOperator(
...
def verify_feas(solution, points, inv_points2, dist_mat, data2, fleet, vehicles):
    ...
    for i in range(len(solution[0])):
        route = solution[0][i]
        feasible = HFRepairOps.feasibility_check(route, points, inv_points2, dist_mat, data2, fleet, vehicles[0][i])
...
while current_iter <= max_iter and current_time < time_limit:
    ...
    chosen_ops = HFOperatorSelection.Roulette_Selection(w_dest, w_rep, w_noise, w_explor, dest_heuristics, rep_heuristics, use_noise, exploration, chosen_ops) 
    teta_dest, teta_rep, teta_noise, teta_explor = increment_teta(teta_dest, teta_rep, teta_noise, teta_explor, chosen_ops)
    ...
    newsol_cost = HFRepairOps.solution_cost(current_sol, dist_mat, points, inv_points2, data, fleet, current_veh_sol)

    if newsol_cost < best_sol_cost:
        pheromone_mat = HFOperatorSelection.Pheromones_update(pheromone_mat, best_sol, 'Local', delta_rho, inv_points2)
    #Acceptance Criteria
    accept_prob = HFAcceptanceCriteria.SimulatedAnnealing(newsol_cost, best_sol_cost, temperature, cooling_rate)
    ...
    if accept:
        ...
        if newsol_cost < global_best_cost:
            ...
            pheromone_mat = HFOperatorSelection.Pheromones_update(pheromone_mat, best_sol, 'Global', delta_rho, inv_points2)
    ...
    #Update scores
    score_dest, score_rep, score_noise, score_explor = HFOperatorSelection.score_update(score_dest, score_rep, score_noise, score_explor, aug_scores, score_case, chosen_ops)         
    ...
    if current_iter % 1000 == 0:
        pheromone_mat = HFOperatorSelection.Pheromones_update(pheromone_mat, best_sol, 'Iter', delta_rho, inv_points2)
    ...
    if current_iter % weight_update_iter == 0:
        ...
        w_dest, w_rep, w_noise, w_explor = HFOperatorSelection.weights_update(score_dest, score_rep, score_noise, score_explor, w_dest, w_rep, w_noise, w_explor, teta_dest, teta_rep, teta_noise, teta_explor, aug_scores, r)
        score_dest = [0]*3
        score_rep = [0]*5
        score_noise = [0]*2
        score_explor = [0]*2
...
global_best_cost = HFRepairOps.solution_cost(global_best, dist_mat, points, inv_points2, data, fleet, global_best_veh)
...
HFPlotsAndResults.SimpleWeightsPlot(iteration, w_evolve)
HFPlotsAndResults.WeightsFillPlot(max_iter, w_evolve)
HFPlotsAndResults.SolutionCostsPlot(iteration, new_sol_cost_evolve, global_sol_cost_evolve, current_sol_cost_evolve)
...
perf_measure_regret2= np.mean(perf_measure_regret2.reshape(-1,int((2*max_iter)/1000)), axis=1)
perf_measure_regret3 = np.array(perf_measure_regret3)
perf_measure_regret3= np.mean(perf_measure_regret3.reshape(-1,int((2*max_iter)/1000)), axis=1)
