...
def preprocessing(df_pu, df_do, df_hub, objects, couriers):
    ...
    point['weight'] = np.zeros(len(point))
    point['volume'] = np.zeros(len(point))
    point = point.reset_index(drop = True)
    for i in range(len(id_list)):
        id_n = id_list[i]
        pu = indices[i+hub_num][0]
        do = indices[i+hub_num][1]
        point['weight'].loc[pu] = objects.loc[id_n][0]
        point['volume'].loc[pu] = objects.loc[id_n][1]
        point['weight'].loc[do] = objects.loc[id_n][0]
        point['volume'].loc[do] = objects.loc[id_n][1]
    for i in range(hub_num):
        point['weight'].loc[i] = np.nan
        point['volume'].loc[i] = np.nan
    for i in range(hub_num, len(indices)):
        point['weight'].loc[indices[i][1]] = point['weight'].loc[indices[i][1]]*-1 
        point['volume'].loc[indices[i][1]] = point['volume'].loc[indices[i][1]]*-1 
