...
def create_route(costumer, dist_mat, indices, hub_num, points, data, inv_points, fleet, veh_solution, pheromone_mat, max_vol):
...
def new_route_insert_list(costumer, dist_mat, indices, hub_num, points, inv_points, data, fleet, vehicle, max_N, chosen_ops):
    ...
    insert_values = (rcost, index, 'nr', 1, vehicle)
        ...
        for i in range(hub_num):
            for route_ind, route in enumerate(partial_solution[i]):
                ...
                for j in range(1, l-1):
                    ...
                    if feasibility_d:
                        insert_list.append((i, route_ind, j))
                    temp_route_p.pop(j)
        if insert_list:
            insert = random.randint(0, len(insert_list)-1)
            i, route_ind, j = insert_list[insert]
            partial_solution[i][route_ind].insert(j, costumer)
        else:
            max_vol = data['weight'][costumer]
            r, ind, veh_solution = create_route(costumer, dist_mat, indices, hub_num, points, data,  inv_points, fleet, veh_solution, pheromone_mat, max_vol)
...
def Greedy_Insertion(hub_num, removed_req, partial_solution, points, data, dist_mat, indices, inv_points, chosen_ops, fleet, veh_solution, pheromone_mat, max_N):
    ...
    for costumer in removed_req:
        ...
        for i in range(hub_num):
            for route_ind, route in enumerate(partial_solution[i]):
                ...
                for j in range(1, l-1):
                    ...
                    feasibility_d = feasibility_check(temp_route_p, points, inv_points, dist_mat, data, fleet, veh_solution[i][route_ind])
                    if feasibility_d:
                        icost = insertion_cost(temp_route_p, rcost, dist_mat, points, inv_points, data, fleet, veh_solution, veh_solution[i][route_ind], max_N, chosen_ops)
                        insert_list.append((icost, i, route_ind, j, 0))
                    temp_route_p.pop(j)   
        if explore == True:
        ...
        if insert_list:
            min_cost, min_i, min_route_ind, min_j, vehicle = heapq.nsmallest(1, insert_list, key=lambda x: x[0])[0]
            if min_route_ind == 'nr':
        ...
        for costumer in removed_req:
            ...
            for i in range(hub_num):
                for route_ind, route in enumerate(partial_solution[i]):
                    ...
                    for j in range(1, l-1):
                        ...
                        if feasibility_d:
                            icost = insertion_cost(temp_route_p, rcost, dist_mat, points, inv_points, data, fleet, veh_solution, veh_solution[i][route_ind], max_N, chosen_ops)
                            route_insert_list.append((icost, i, route_ind, j, 0))
                        temp_route_p.pop(j)
            ...
            if len(insert_list) > regret: 
                if regret == 1:
                    ...
                    min_cost, min_i, min_route_ind, min_j, vehicle = insert_sorted[0]
                    regret_values.append((insert_sorted[regret][0] - min_cost, min_i, min_route_ind, min_j, vehicle, costumer))
                if regret == 2:
                    insert_sorted = heapq.nsmallest(regret+1, insert_list, key=lambda x: x[0])
                    min_cost, min_i, min_route_ind, min_j, vehicle = insert_sorted[0]
                    regret_values.append(((insert_sorted[regret][0] - min_cost)+(insert_sorted[regret-1][0] - min_cost), min_i, min_route_ind, min_j, vehicle, costumer))
    
        if regret_values:
            sorted_array = sorted(regret_values, key=lambda x: x[0], reverse=True)
            min_cost, min_i, min_route_ind, min_j, vehicle, costumer = sorted_array[0]
            if min_route_ind == 'nr':
                r, ind, veh_solution = create_veh_route(costumer, dist_mat, indices, hub_num, points, data, inv_points, fleet, veh_solution, vehicle)
                partial_solution[ind].append(r)
            else:
